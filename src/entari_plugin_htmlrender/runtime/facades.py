"""Lifecycle-bound facades for services exposed by :class:`RenderRuntime`."""
...
if TYPE_CHECKING:
    ...
    from entari_plugin_htmlrender.resources.service import (
        ResolverSpec,
        ResourceService,
    )
...
class RuntimeResources(Protocol):
    """Resource operations exposed through a render runtime's lifetime."""
...
class AdmittedResourceService:
    """Keep public resource access from outliving its runtime caches."""
    ...
    async def resolve_resource_url(
        ...
        *,
        ...
        async with self._admission.operation():
            return await self._delegate.resolve_resource_url(
                value,
                template_base=template_base,
                strict=strict,
                resolver=resolver,
