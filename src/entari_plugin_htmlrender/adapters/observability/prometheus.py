...
from .common import OptionalModuleLoader
...
_PROM_FILEHOST_ACTIVE_MAPPINGS_NAME = "entari_htmlrender_filehost_active_url_mappings"
...
_module_loader = OptionalModuleLoader("prometheus_client")


def ensure_prometheus_available(*, reason: str) -> bool:
    """Return whether the optional Prometheus client is importable."""
...
def _load_prometheus() -> tuple[Counter, Histogram] | None:
    ...
    try:
        ...
        prometheus = _module_loader.load(reason="runtime")
        ...
        logger.warning(
            "Prometheus provider initialization failed: %s.",
            error,
        )
        return None
...
def record_metrics(
    ...
    try:
        ...
        logger.warning(
            "Prometheus metric export failed: %s.",
            error,
        )
...
def _load_filehost_metrics_unlocked() -> (
    ...
    try:
        ...
        prometheus = _module_loader.load(reason="filehost_metrics")
        if prometheus is None:
            return None
        counter_cls = getattr(prometheus, "Counter", None)
        ...
        logger.warning(
            "Prometheus filehost metric initialization failed: %s.",
            error,
        )
        return None
...
def record_filehost_cache_metrics(
    ...
    try:
        ...
        logger.warning(
            "Prometheus filehost metric export failed: %s.",
            error,
        )
...
def _load_cache_metrics_unlocked() -> tuple[Counter, Gauge, Gauge] | None:
    try:
        ...
        prometheus = _module_loader.load(reason="cache_metrics")
        if prometheus is None:
            return None
        counter_cls = getattr(prometheus, "Counter", None)
        gauge_cls = getattr(prometheus, "Gauge", None)
        ...
        logger.warning(
            "Prometheus cache metric initialization failed: %s.",
            error,
        )
        return None
...
def record_cache_metrics(
    ...
    try:
        ...
        logger.warning(
            "Prometheus cache metric export failed: %s.",
            error,
        )
