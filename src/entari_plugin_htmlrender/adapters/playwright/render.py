    ...
    def _resolve_mode(self) -> PlaywrightMode:
        ...
        if has_cdp and has_ws:
            raise RuntimeError(
                ...
                "`provider_config.connect_cdp.endpoint` and "
                "`provider_config.connect_ws.endpoint` cannot both be set."
            )
        if has_cdp:
        ...
        match mode:
            case PlaywrightMode.REMOTE_CDP:
                if cfg.engine is not BrowserEngine.CHROMIUM:
                    raise RuntimeError(
                        'CDP connection requires `provider_config.engine="chromium"`.'
                    )
    ...
    def _probe_ws_paths(cls, base: str) -> tuple[int, int, int] | None:
        ...
        for path in ("/json/version", "/"):
            try:
                ...
                logger.debug(
                    "WS version probe request failed: %s",
                    type(error).__name__,
                )
                continue
