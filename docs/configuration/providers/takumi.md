...
| `provider_config.html_options.presets` | `chromium` | `chromium` 或 `none` |
...
只有随镜像不可变的字体才应使用 `immutable`。运行中的 native renderer不会热替换已注册字体；文件变化后需重建 composition。字体路径同样受`resources.local_access.allowed_paths` 约束。
...
统计是当前 runtime 的只读快照。公共 API 不提供 compiled cache clear；需要释放 native compiled object 或替换已注册字体时，由 Entari 关闭并重新加载 service。字体 revalidate、native image cache 与完整调优流程见[缓存组件与调优](../../guides/cache-lifecycle.md)。
